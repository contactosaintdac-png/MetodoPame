...
- **Sprint de Estabilização e Validações Gerais** — Lançamento da suite de validação e roteamento seguro de sessões:
  ...
  - **Painel da Especialista:** Agenda mensal com modo Calendário, configuração de disponibilidade em pílulas táteis semanais, edição protegida de perfil via Firestore `pendingUpdate` e integração do canal de ajuda via WhatsApp.
